...
EOF
  ...
  if [ -f "$report_dir/slither.json" ]; then
    ...
    printf '### 🐍 Slither\n\n' >> "$TMP_BODY"
    printf '**%s findings** — 🔴 High: %s · 🟡 Medium: %s · 🔵 Low/Info: %s\n\n' "$TOTAL" "$HIGH" "$MED" "$LOW" >> "$TMP_BODY"
  ...
  if [ -f "$report_dir/cargo_audit.json" ]; then
    ...
    [ "$NOTES" -gt 0 ] 2>/dev/null && printf ' · ⚠️ %s notes' "$NOTES" >> "$TMP_BODY"
    printf '\n\n' >> "$TMP_BODY"
    ...
    if [ "$NOTES" -gt 0 ] 2>/dev/null; then
      jq -r '.notes[]? | "> ⚠️ **\(.status)**: \(.note // .error)"' \
  ...
  printf '\n---\n*Generated by [PR Auditor](https://github.com/Shawnchee/the-auditor) 🔬 — Raw Tool Summary*\n' >> "$TMP_BODY"
